...
function AddParticipantModal({ isOpen, onClose, onAdd }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 px-3 py-4 sm:p-4">
      <div className="bg-dark-800 border border-matrix-green/30 rounded-xl w-full max-w-md max-h-[80dvh] sm:max-h-[70vh] flex flex-col">
        <div className="flex items-center justify-between p-3 sm:p-4 border-b border-gray-800">
          <h3 className="text-base sm:text-lg font-semibold text-matrix-green">
            Add Participant
          </h3>
...
export function ActiveCallView() {
  ...
  return (
    ...
    >
        ...
        {isVideoCall ? (
            ...
            {isConference && gridLayout === "grid" ? (
              ...
              <>
                ...
                <div
                  className={`absolute transition-all duration-300 cursor-pointer ${
                    localVideoEnlarged
                      ...
                      : "bottom-24 sm:bottom-28 right-2 sm:right-4 w-24 sm:w-36 md:w-44 aspect-[3/4] rounded-xl"
                ...
                {isConference && gridLayout === "speaker" && (
                  <div className="absolute top-2 sm:top-4 left-2 sm:left-4 right-16 sm:right-20 flex gap-1.5 sm:gap-2 overflow-x-auto pb-2 scrollbar-hide">
                    {allParticipants.map((participant) => (
                      <div
                        key={participant}
                        onClick={() => setEnlargedParticipant(participant)}
                        className="flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 rounded-lg overflow-hidden border-2 border-dark-600 hover:border-matrix-green cursor-pointer bg-dark-800"
                      >
                        <div className="w-full h-full flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center px-3 sm:px-4">
            {isConference ? (
              <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-4 sm:mb-6">
                {allParticipants.map((participant) => (
                  <div key={participant} className="flex flex-col items-center">
                    <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-full bg-matrix-green/20 border-2 border-matrix-green flex items-center justify-center shadow-lg shadow-matrix-green/30">
                      <span className="text-xl sm:text-2xl md:text-3xl text-matrix-green font-bold">
                        {participant?.charAt(0).toUpperCase()}
                      </span>
                    </div>
                    <span className="mt-1.5 sm:mt-2 text-xs sm:text-sm text-white truncate max-w-[5rem]">
                      {participant}
                    </span>
                  </div>
                ))}
              ...
              <>
                <div className="w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-full bg-matrix-green/20 border-4 border-matrix-green flex items-center justify-center mb-3 sm:mb-4 shadow-lg shadow-matrix-green/30">
                  <span className="text-3xl sm:text-4xl md:text-5xl text-matrix-green font-bold">
                    {activeCall.targetAvatar?.charAt(0).toUpperCase()}
                  </span>
                </div>
                <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-2 text-center truncate max-w-full px-4">
                  {activeCall.targetAvatar}
                </h3>
              </>
            )}
      ...
      {/* Controls */}
      <div className="flex-shrink-0 p-3 sm:p-4 md:p-6 bg-dark-800/90 backdrop-blur-sm border-t border-gray-800">
        <div className="flex items-center justify-center gap-2 sm:gap-3 md:gap-4 max-w-lg mx-auto">
          {/* Mute Button */}
          <button
          ...
          >
            {isMuted ? (
              <MicOff className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-red-500" />
            ) : (
          ...
          >
            {!isVideoEnabled ? (
              <VideoOff className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-red-500" />
            ) : (
          ...
          <button
            ...
            className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-full bg-red-500 flex items-center justify-center hover:bg-red-600 active:scale-95 transition-all duration-200 shadow-lg shadow-red-500/30"
            title="End Call"
          >
            <PhoneOff className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-white" />
          </button>

          {/* Add Participant */}
          <button
            onClick={() => setShowAddParticipant(true)}
            className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-full bg-dark-700 border-2 border-dark-500 flex items-center justify-center hover:bg-matrix-green/20 hover:border-matrix-green transition-all duration-200"
          ...
          <button
            ...
            className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-full bg-dark-700 border-2 border-dark-500 flex items-center justify-center hover:bg-dark-600 hover:border-gray-500 transition-all duration-200"
            title="Switch Camera"
          >
            <SwitchCamera className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white" />
          </button>
        </div>
